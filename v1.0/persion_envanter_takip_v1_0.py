...
# Temalı bir Tkinter penceresi oluşturma
root = ThemedTk(theme="adapta")  # 'Arc', 'equilux' vb. farklı temalar seçebilirsiniz.
root.title("PersioN Envanter Takip Programı v1.0")

# ico = Image.open('persion_icon.png')
...
# Seçilenöğeyi düzenleme fonksiyonu
def oge_duzenle():
    ...
    class EditDialog(Dialog):
        def body(self, master):
            self.title("Düzenle")
            tk.Label(master, text="Ürün Tanımı:").grid(row=0, sticky="w")
            tk.Label(master, text="Seri No:").grid(row=1, sticky="w")
            tk.Label(master, text="Parça No:").grid(row=2, sticky="w")
            tk.Label(master, text="Adet:").grid(row=3, sticky="w")

            self.new_urun_tanimi = tk.Entry(master)
            self.new_urun_tanimi.grid(row=0, column=1)
            self.new_urun_tanimi.insert(0, item_data[0])

            self.new_seri_no = tk.Entry(master)
            self.new_seri_no.grid(row=1, column=1)
            self.new_seri_no.insert(0, item_data[1])

            self.new_parca_no = tk.Entry(master)
            self.new_parca_no.grid(row=2, column=1)
            self.new_parca_no.insert(0, item_data[2])

            self.new_adet = tk.Entry(master)
...
urun_tanimi_label.pack()
urun_tanimi_entry = ttk.Entry(root)
urun_tanimi_entry.pack()
